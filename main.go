...
// SlackPayload describes the body to send to Slack.
...
// SendSlack send SlackPayload body via webHookURL.
func SendSlack(pl *SlackPayload, webHookURL string) (*http.Response, error) {
	jo, err := json.Marshal(pl)
	if err != nil {
		return nil, err
	}
	...
	req, err := http.NewRequest(
		"POST",
		webHookURL,
		bytes.NewBuffer([]byte(string(jo))),
	)
	if err != nil {
		return nil, err
	}
